...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserBySsoAuthenticationDetails(SsoAuthenticationDetails ssoAuthenticationDetails) {
        ...
        if(certificationFlag) {
            UsersList commonGetUsers = restTemplateService.send(Constants.TARGET_COMMON_API, "/users/serviceInstanceId/" + serviceInstanceId + "/spaceGuid/" + space_guid, HttpMethod.GET, null, UsersList.class);
            ...
            if(commonGetUsers != null && commonGetUsers.getItems().size() > 0) {
                ...
                if(commonGetUsers.toString().contains("userId="+username+",")) {
                    ...
                    if(username.contains("@")) {
                        ...
                        userName = (space_guid + "-" + tmpString[0].replaceAll("([:.#$&!_\\(\\)`*%^~,\\<\\>\\[\\];+|-])+", "")).toLowerCase() + "-user";
                        roleName = (tmpString[0].replaceAll("([:.#$&!_\\(\\)`*%^~,\\<\\>\\[\\];+|-])+", "")).toLowerCase();
                    } else {
                        userName = (space_guid + "-" + username.replaceAll("([:.#$&!_\\(\\)`*%^~,\\<\\>\\[\\];+|-])+", "")).toLowerCase() + "-user";
                        roleName = (username.replaceAll("([:.#$&!_\\(\\)`*%^~,\\<\\>\\[\\];+|-])+", "")).toLowerCase();
                    }
